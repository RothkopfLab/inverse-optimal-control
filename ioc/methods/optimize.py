...
def minimize(fun, x0,
    ...
    adapted from https://gist.github.com/slinderman/24552af1bdbb6cb033bfea9b2dc4ecfd
    ...
    if bounds is not None:
        bounds_flat, _ = tree_flatten(bounds)
        bounds = np.array(bounds_flat).reshape((-1, 2))
...
def minimize_bobyqa(fun, x0, bounds=None, tol=1e-14):
    x0_flat, unravel = ravel_pytree(x0)
    if bounds is not None:
        bounds_flat, _ = tree_flatten(bounds)
        bounds_arr = np.array(bounds_flat).reshape((-1, 2)).T
    ...
    res = pybobyqa.solve(fun_wrapper, np.array(x0_flat), bounds=bounds, rhoend=tol, scaling_within_bounds=True,
                         print_progress=False, do_logging=False)
    res.x = unravel(res.x)
